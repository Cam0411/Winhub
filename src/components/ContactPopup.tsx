...
export default function ContactPopup({ isOpen, onClose }: ContactPopupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const apiKey = 'c9157a71bef943eebc64c3409b81a8e9';
      const workspaceId = '2278';

      const response = await fetch(`https://crm.pancake.vn/api/workspaces/${workspaceId}/contact/records?api_key=${apiKey}`, {
        method: 'POST',
        headers: {
                  ...
                  <form className="space-y-5" onSubmit={handleSubmit}>
                    ...
                    <div className="relative">
                      <select 
                        ...
                        onChange={handleChange}
                        className="w-full bg-white/[0.03] border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-blue-500/50 focus:bg-white/[0.06] transition-all appearance-none cursor-pointer"
                      >
                        <option value="art-branding">Bạn quan tâm dịch vụ nào?</option>
                        <option value="art-branding">Art Branding Website</option>
                        <option value="conversion">Conversion Performance</option>
